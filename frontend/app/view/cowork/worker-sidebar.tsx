...
export function WorkerSidebar({ workers, onClose, onCreateWorker, onDeleteWorker }: WorkerSidebarProps) {
    ...
    const [timeout, setTimeout_] = React.useState(300);
    ...
    const handleSelectWorker = (workerId: string) => {
        ...
        setIsNew(false);
        setTool(w.tool ?? "claude");
        setName(w.name ?? "");
        setConcurrency((w as any).concurrency ?? 3);
        setTimeout_((w as any).timeout ?? 300);
        setMaxRetries((w as any).maxretries ?? 3);
        setCapabilities(w.capabilities ?? []);
        setRole(w.role ?? "");
        setDesc(w.desc ?? "");
        setCustomCmd(w.customcmd ?? "");
        setSoul(w.soul ?? "");
        setSkills(w.skills ?? "");
        setMcpServers(w.mcpservers ?? "");
    };
                ...
                <div className="flex-1 overflow-y-auto py-1">
                    {workers.map((w) => (
                        ...
                        >
                            ...
                            <div className="text-[10px] text-muted-foreground mt-0.5 pl-3">{w.tool}</div>
                        </button>
                    ))}
                ...
                {!showDetail ? (
                    ...
                    <div className="p-4 space-y-4">
                        ...
                        {!isNew && selectedWorker && (
                            <div>
                                <label className="text-[11px] text-muted-foreground mb-0.5 block">Runtime</label>
                                <span className="text-sm text-secondary">{selectedWorker.tool}</span>
                            </div>
                        )}
                        ...
                        <div className="flex gap-3">
                            <label className="flex items-center gap-1 text-xs text-muted-foreground">
                                Concurrency <input type="number" className={inputSmCls} value={concurrency} onChange={(e) => setConcurrency(Number(e.target.value))} />
                            </label>
                            <label className="flex items-center gap-1 text-xs text-muted-foreground">
                                Timeout <input type="number" className={inputSmCls} value={timeout} onChange={(e) => setTimeout_(Number(e.target.value))} />s
                            </label>
                            <label className="flex items-center gap-1 text-xs text-muted-foreground">
                                Retries <input type="number" className={inputSmCls} value={maxRetries} onChange={(e) => setMaxRetries(Number(e.target.value))} />
                            </label>
                        </div>
                        ...
                        {/* Role */}
                        <div>
                            <label className="text-[11px] text-muted-foreground mb-0.5 block">Role</label>
