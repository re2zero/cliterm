...
const MEMBER_TEMPLATES: Record<string, Omit<MemberConfigResult, "capabilities"> & { label: string; description: string }> = {
	...
	backend: { label: "Go Backend", description: "Go backend dev", runtime: "claude", concurrency: 2, timeout: 300, maxRetries: 3 },
	frontend: { label: "Frontend Dev", description: "React/Tailwind", runtime: "claude", concurrency: 2, timeout: 300, maxRetries: 3 },
				...
				<div className="grid grid-cols-3 gap-2">
					{(Object.entries(MEMBER_TEMPLATES)).map(([key, t]) => (
						...
						>
							...
							<span className="text-tertiary block mt-0.5">{t.description}</span>
						</button>
					))}
				</div>
			</div>
