# Phase1B: pkg/team/team_inject.go — CLI config injection
...
Implement CLI configuration injection for Workers. Includes: InjectWorkerConfig (dispatch to tool-specific injection), loadPersona (read personaPath or inline), linkSkills/unlinkSkills (symlink management), injectMCP (MCP config files per CLI tool).

## Files to Modify

- pkg/team/team_inject.go
