# Phase1D: AI tools (tools_team.go) + system prompt
...
Create pkg/aiusechat/tools_team.go with 15 team_* AI tools (team_fork_worker, team_list_workers, team_list_members, team_create_member, team_update_member, team_delete_member, team_create_task, team_assign_task, team_update_task, team_get_status, team_execute_task, team_recycle_worker, team_send_prompt, team_get_task_output, team_list_activity). Follow callback pattern from tools_cowork.go. Also update usechat-prompts.go (CoworkMode→TeamMode prompt), usechat.go, and uctypes/uctypes.go.

## Files to Modify

- pkg/aiusechat/tools_team.go
